...
## 功能特性
- **多数据源**：拖拽/选择本地 CSV、载入/下载示例，或浏览 `public/server-data` 下的服务器文件。
...
## 环境变量
### S3 模式（可选）
设置 `S3_SERVER_DATA_ROOT=s3://bucket/prefix/` 后，服务器浏览与 CSV 内容会从 S3 前缀读取；同时 `/server-data/**` 会 302 跳转到对应对象的签名 URL（便于 CSV 引用图片等资源）。

常用配置：
- `S3_SERVER_DATA_ROOT`：`s3://bucket/prefix/`
- `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` / `AWS_REGION`（或 `S3_REGION`）
- 可选：`S3_ENDPOINT`（S3 兼容服务，如 MinIO），`S3_FORCE_PATH_STYLE=true`
- 可选：`S3_PRESIGN_EXPIRES`（签名 URL 过期秒数，默认 `300`）
...
## 部署
1. 运行 `yarn build` 生成 `.next`.
2. 选择托管方式：
   - **Vercel / Netlify / Railway**：直接导入仓库即可，默认 Node.js 18。
   - **自托管**：将产物与 `package.json` 一并上传，执行 `yarn start`，或封装到 Docker/PM2。
3. 数据来源二选一：
